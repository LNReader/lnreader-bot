...
export function getLocalizedInfo(target: 'NAME' | 'DESCRIPTION', localizationSource: TranslationsNestedPaths) {
	const localizations = Object.fromEntries(
		locales
			.map(locale => [
				locale,
				getLocalizationFromPathString(`${localizationSource}.${target}` as TranslationsNestedPaths, locale),
			])
...
export function setFallbackDescription<K extends SanitizedOptions>(options: K & { description?: string }) {
	...
	for (const locale of locales) options.descriptionLocalizations[locale] = L[locale].SHARED.NO_COMMAND_DESCRIPTION()
